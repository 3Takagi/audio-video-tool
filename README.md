...
音视频工具是一个运行在 Windows 本机的媒体工具箱，支持图片超分、YouTube 视频下载、bilibili 视频下载和视频封面下载。
...
| 便携网页版 | `AudioVideoTool-Portable-Python.zip` | 不想安装软件、希望解压即用的用户 |
...
- YouTube 视频下载：基于 yt-dlp，默认选择最高可用画质，也支持指定上限清晰度。
...
`AudioVideoTool.html` 只是快捷入口页，不能启动 Python 后端。它只能查找并跳转到已经运行中的本地服务。如果打不开，请先运行 `start.bat`。
...
可以。桌面安装版已经包含桌面壳和后端基础资源，不需要再下载 zip。
...
不建议。YouTube / bilibili 下载涉及平台条款、版权和账号安全问题。本项目默认定位为本机个人工具。
